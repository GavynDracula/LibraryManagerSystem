...
#include "ui_admininquiredialog.h"

AdminInquireDialog::AdminInquireDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::AdminInquireDialog)
{
    int i;

    ui->setupUi(this);
    this->setWindowTitle("Inquire the books you want to borrow");
...
AdminInquireDialog::~AdminInquireDialog()
{
    delete ui;
}

int AdminInquireDialog::myExec(QSqlQuery *query)
{
    this->query = query;
    return exec();
}

int AdminInquireDialog::SetSelectQuery()
{
    int i;
    QString key1;
    QString key2;
...
void AdminInquireDialog::AddCondition()
{
    if (ConditionNum < 7)
    {
        ConditionNum += 1;
...
void AdminInquireDialog::PlusCondition()
{
    if (ConditionNum > 0)
    {
...
void AdminInquireDialog::PubdateCondition(int state)
{
    if (state == Qt::Checked)
    {
...
void AdminInquireDialog::PagesCondition(int state)
{
    if (state == Qt::Checked)
    {
...
void AdminInquireDialog::ShowConditionSet(int conditionNum)
{
    switch (conditionNum)
    {
        case 1:
...
void AdminInquireDialog::HideConditionSet(int conditionNum)
{
    switch (conditionNum)
    {
        case 1:
